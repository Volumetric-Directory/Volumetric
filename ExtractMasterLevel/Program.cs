...
namespace ExtractMasterLevel
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            float? Mlevel = AudioManager.GetMasterVolume();
            sw.Write(Mlevel);
            //sw.Write(",");
            /* This following commented code is unworking
            int appNumber = 1;
            float? AppLevel = 0;
            while (appNumber < 6)
            {
                string app = "dummyapp"; //Declaring a Name here is just for debug to make sure it gets updated.
                var pathParse = @"C:\Volumetric\names.csv"; //Path for the file
                ...
                foreach (var process in Process.GetProcesses())
                {
                    if (process.ProcessName == app)
                    {
                        ...
                        AppLevel = AudioManager.GetApplicationVolume(pID);
                        appNumber++;
                        if (AppLevel != null)
                        {
                            sw.Write(AppLevel);
        ...
        public static class AudioManager
        {
            ...
            public static float? GetApplicationVolume(int pid)
            {
                ...
                float level;
                volume.GetMasterVolume(out level); //THIS IS NOT the function defined above, it is the API function from ISimpleAudio
