...
namespace VolumeControlApp
{
    class Program
    {
        ...
        {
            //Get information about what app we're adjusting
            string line="8"; //default to zero (Master Volume)
            try
            {
                //Pass the file path and file name to the StreamReader constructor
                StreamReader sr = new StreamReader("number.txt");
                //Read the first line of text. That's the only line we need read.
                line = sr.ReadLine();
                sr.Close();  //Close the file
            }
            catch (Exception e)
            {
                Console.WriteLine("Exception: " + e.Message); //Copied error handling from microsoft.com
            }
            int AppNumber = int.Parse(line); //Convert the read-in string to an int for use in program.
            Console.WriteLine(AppNumber); //Debugging


            string Slevel = "7"; //The new volume level, S for string, 7 for debugging
            try
            {
                //Pass the file path and file name to the StreamReader constructor
            ...
            float level = float.Parse(Slevel); //Convert the read-in string to an float for use in program.
            Console.WriteLine(level); //Debugging



            if (AppNumber > 5)
                return; //Full stop if we don't have a valid number
            else if (AppNumber != 0)
            {
                string app = "dummyapp"; //Declaring a Name here is just for debug to make sure it gets updated.
                var pathParse = "names.csv"; //Path for the file
                using (TextFieldParser csvParser = new TextFieldParser(pathParse))
                {
                    ...
                    csvParser.ReadLine();
                    int i = 0;
                    while (i < AppNumber) //March through to get to the needed line
                    {
                        if (!csvParser.EndOfData)
                        {
                            // Read current line fields, pointer moves to the next line.
                            string[] fields = csvParser.ReadFields();
                            app = fields[0];  //store the name of the app we're adjusting
                            string Address = fields[1]; //Unneeded for present use, safe to ignore
                            i++;
                        }
                    }
                }
                Console.WriteLine(app); //Confirm we did the above correctly

                var pID = 0;
                foreach (var process in Process.GetProcesses())
                {
                    if (process.ProcessName == app)
                    {
                        pID = process.Id;
                        Console.WriteLine(pID); //Just a debugging measure. Writes all the processID #s of the named app to console
                        if (pID != 0) //Don't try to change a process with no ID's volume
                            AudioManager.SetApplicationVolume(pID, level);
                    }
                }

                if (pID == 0)
                    return; //Don't try to change a process with no ID's volume
            }
            else if (AppNumber == 0)
            {
